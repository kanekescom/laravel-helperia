...
class TransCommand extends Command
{
     ...
     * @var string
     */
    public $signature = 'helperia:trans
    ...
    {
        $stats = Trans::stats($translations);
    ...
    {
        $duplicates = Trans::duplicates($jsonContent);
    ...
    {
        $untranslated = Trans::untranslated($translations);
    ...
    {
        if (! Trans::hasDuplicates($jsonContent)) {
        ...
        $duplicates = Trans::duplicates($jsonContent);
        $translations = Trans::removeDuplicates($jsonContent);
        Trans::save($file, $translations, false); // Don't sort
    ...
    {
        Trans::save($file, $translations, true); // Sort keys
    ...
    {
        $untranslated = Trans::untranslated($translations);
        $translated = Trans::translated($translations);
